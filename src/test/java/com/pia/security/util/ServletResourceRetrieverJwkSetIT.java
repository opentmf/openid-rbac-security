...
class ServletResourceRetrieverJwkSetIT {
  ...
  void testRetrieveResource() throws IOException {
    ...
    Assertions.assertTrue(jwtService.isExpiredToken(TokenUtil.EXPIRED_TOKEN));
    Assertions.assertFalse(jwtService.isExpiredToken(TokenUtil.READ_TOKEN));
    Assertions.assertFalse(jwtService.isExpiredToken(TokenUtil.WRITE_TOKEN));
  }
}
